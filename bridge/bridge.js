...
export function setupDataWS() {
  ...
  dataWS = new WebSocket(`${webSocketURL()}/data`);
  console.log("setupDataWS", dataWS);
...
export function goIntoSessions() {
...
export function createNewSession() {
  const loc = window.location.toString();
  const index = loc.indexOf("/sessions/");
  if (index < 0) {
    // there is no sessions (space) yet
    return Promise.resolve(false);
  }

  if (!/\/sessions\/?$/.test(loc)) {
    // does not end with "/sessions/"", meaning that there already is a session
  }

  const semiIndex = loc.indexOf(";");
  const base = loc.slice(semiIndex, index + "/sessions/".length);

  const url = `https://substrate.home.arpa/bridge2${base}new`;
